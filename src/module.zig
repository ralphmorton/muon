...
test "init empty module" {
    ...
    const binary = try std.fs.cwd().readFileAlloc("test/empty.wasm", allocator, std.Io.Limit.unlimited);
...
test "init trivial module" {
    ...
    const binary = try std.fs.cwd().readFileAlloc("test/trivial.wasm", allocator, std.Io.Limit.unlimited);
    var reader = std.Io.Reader.fixed(binary);
    defer allocator.free(binary);
...
test "funcs" {
    ...
    const binary = try std.fs.cwd().readFileAlloc("test/funcs.wasm", allocator, std.Io.Limit.unlimited);
    var reader = std.Io.Reader.fixed(binary);
    defer allocator.free(binary);
...
test "add" {
    ...
    const binary = try std.fs.cwd().readFileAlloc("test/add.wasm", allocator, std.Io.Limit.unlimited);
    var reader = std.Io.Reader.fixed(binary);
    defer allocator.free(binary);
